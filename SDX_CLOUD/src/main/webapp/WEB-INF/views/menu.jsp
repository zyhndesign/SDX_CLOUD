<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<ul class="menu">
	
	<li class="item"><span class="glyphicon glyphicon-th-large sdxIcon"></span> <a class="link">功能</a> 
		<ul class="subMenu">
			<li class="sItem">
				 <a class="sLink" href="hpgl/hpManage/productMgr">货品管理</a>
			</li>
		</ul></li>
	<li class="item"><span class="glyphicon glyphicon-cog sdxIcon"></span> <a class="link">基础设置</a> 
		<ul class="subMenu">
			<li class="sItem">
				<a class="sLink" href="hpgl/brand/brandMgr">品牌系列设置</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="hpgl/category/categoryMgr">品类设置</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="hpgl/size/sizeMgr">尺码设置</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="hpgl/datetime/seasonMgr">时间设置</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="hpgl/color/colorMgr">颜色设置</a>
			</li>
		</ul></li>

	<li class="item"><span class="glyphicon glyphicon-book sdxIcon"></span> <a class="link">管理</a> 
		<ul class="subMenu">
			<li class="sItem">
				<a class="sLink" href="dggl/shop/shopMgr">店铺管理</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="dggl/appUser/guideMgr">导购管理</a>
			</li>
			<li class="sItem">
				<a class="sLink" href="dggl/appVipUser/vipCustomerMgr">VIP客户管理</a>
			</li>
		</ul></li>
</ul>